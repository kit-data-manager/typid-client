...
> NOTE This is in an early stage. Do not yet use in production!

This is a very early experiment. There is a lot to do:

- [ ] See if we can have the generated code as a full, non-published subproject that we depend on. Then expose the nice parts and high-level additions of our wrapper.
- [ ] The setup is quite verbose, with gitlab+github support etc, but for now we should only keep what we really want to maintain.
    - [ ] CI stuff
    - [ ] scripts
    - [ ] not sure if we still need a setup.py. I'd prefer all metadata only once if we really do not need it. See https://packaging.python.org/en/latest/guides/modernize-setup-py-project/
